...
Do not set agency_other high merely because the speaker said the utterance. Set agency_other high only when the listener would attribute meaningful responsibility, blame, or credit to the speaker for the situation.
...
def get_thread_client(api_key: str, base_url: str) -> OpenAI:
    """
    Creates one OpenAI-compatible client per worker thread.
    This avoids sharing one client object across many threads.
    """
...
def process_one_line(
    ...
    """
    Worker function for one JSONL line.
    Returns a result dictionary instead of writing files directly.
    File writes stay in the main thread to avoid corrupted JSONL output.
    """
    ...
    try:
        ...
        return {
            "ok": True,
            "line_idx": line_idx,
            "output_line": json.dumps(labeled_sample, ensure_ascii=False),
        }
...
def process_jsonl(
    ...
    """
    Processes JSONL samples concurrently with a thread pool.

    Important design choice:
    - API calls happen in worker threads.
    - File writes happen only in the main thread.
    - Results are written in original input order, not completion order.
    """
    ...
    # Collect the lines that need processing.
    # This keeps the output ordering logic simple and reliable.
    ...
    print(f"Queued samples:  {total_to_process}")
    print()
    ...
    # Results may complete out of order, so buffer them until the next expected
    # input line is ready to write.
    results_by_line_idx: Dict[int, Dict[str, Any]] = {}
    task_order = [line_idx for line_idx, _ in tasks]
    next_write_pos = 0

    with output_path.open(output_mode, encoding="utf-8") as fout, \
        ...
        future_to_line_idx = {
            ...
            for line_idx, line in tasks
        }

        for future in as_completed(future_to_line_idx):
            line_idx = future_to_line_idx[future]

            try:
                result = future.result()
            except Exception as e:
                # This is a safety net. process_one_line should normally catch
                # per-sample exceptions and return a structured failed result.
                result = {
                    "ok": False,
                    "line_idx": line_idx,
                    "error": repr(e),
            ...
            results_by_line_idx[line_idx] = result
            completed += 1

            # Write any consecutive completed results in original input order.
            while next_write_pos < len(task_order):
                expected_line_idx = task_order[next_write_pos]

                if expected_line_idx not in results_by_line_idx:
                    break

                ordered_result = results_by_line_idx.pop(expected_line_idx)

                if ordered_result["ok"]:
                    fout.write(ordered_result["output_line"] + "\n")
                    fout.flush()
                    written += 1
                    print(
                        f"[OK] input_line={expected_line_idx} "
                        f"completed={completed}/{total_to_process} written={written} failed={failed}"
                    )
                else:
                    ffailed.write(
                        json.dumps(ordered_result["fail_record"], ensure_ascii=False) + "\n"
                    )
                    ffailed.flush()
                    failed += 1
                    print(
                        f"[FAILED] input_line={expected_line_idx} "
                        f"completed={completed}/{total_to_process} error={ordered_result['error']}"
                    )
    ...
    print()
    print("========== Done ==========")
    print(f"New samples processed: {total_to_process}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of parallel API worker threads. Start with 4; reduce if rate-limited.",
    )
